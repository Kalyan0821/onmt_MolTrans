...
from moltrans_onmt.modules.sparse_losses import SparsemaxLoss
...
def build_loss_compute(model, tgt_vocab, opt, train=True):
    ...
    """
    device = torch.device("cuda" if moltrans_onmt.utils.misc.use_gpu(opt) else "cpu")

    if opt.copy_attn:
        compute = moltrans_onmt.modules.CopyGeneratorLossCompute(
            model.generator, tgt_vocab, opt.copy_attn_force, opt.copy_loss_by_seqlength
        )
    else:
...
class LossComputeBase(nn.Module):
    ...
    def monolithic_compute_loss(self, batch, output, attns):
        ...
        Returns:
            :obj:`moltrans_onmt.utils.Statistics`: loss statistics
        """
    ...
    def sharded_compute_loss(
        ...
        Returns:
            :obj:`moltrans_onmt.utils.Statistics`: validation loss statistics

        """
        batch_stats = moltrans_onmt.utils.Statistics()
    ...
    def _stats(self, loss, scores, target):
        ...
        Returns:
            :obj:`moltrans_onmt.utils.Statistics` : statistics for this batch.
        """
        pred = scores.max(1)[1]
        non_padding = target.ne(self.padding_idx)
        num_correct = pred.eq(target).masked_select(non_padding).sum().item()
        num_non_padding = non_padding.sum().item()
        return moltrans_onmt.utils.Statistics(loss.item(), num_non_padding, num_correct)
