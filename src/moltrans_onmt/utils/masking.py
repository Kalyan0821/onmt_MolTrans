...
class ChemVocabMask(object):
    ...
    def _get_valid_tokens_per_src_seq_in_batch(self, src):
        valid_tokens_per_seq = [
            np.unique(
                [
                    vocab_list
                    for voc in np.unique(s.cpu().numpy())
                    for vocab_list in self.vocab_vocab_dict[voc]
                ]
            )
            for s in src.t()
        ]
    ...
    def get_log_probs_masking_tensor(self, src, beam_size):
        ...
        mask = torch.stack(
            [
                (
                    torch.ones(len(self.vocab)).index_fill(
                        0, torch.tensor(valid_tokens), 0
                    )
                    * 1e15
                ).index_fill(0, torch.tensor(valid_tokens), 1)
                for valid_tokens in valid_tokens_per_seq
    ...
    def _get_unique_vocab_counts_from_source(self, src):
        unique_counts_dicts = [
            dict(zip(*np.unique(s.cpu().numpy(), return_counts=True))) for s in src.t()
        ]
