...
class AudioEncoder(nn.Module):
    ...
    def __init__(
        ...
        for layer_idx in range(enc_layers - 1):
            ...
            setattr(self, "rnn_%d" % (layer_idx + 1), rnn)
            setattr(
                self,
                "pool_%d" % (layer_idx + 1),
                nn.MaxPool1d(enc_pooling[layer_idx + 1]),
            )
            setattr(self, "batchnorm_%d" % (layer_idx + 1), batchnorm)

    def forward(self, src, lengths=None):
        "See :obj:`moltrans_onmt.encoders.encoder.EncoderBase.forward()`"
        ...
        for layer_idx in range(self.enc_layers):
            rnn = getattr(self, "rnn_%d" % layer_idx)
            pool = getattr(self, "pool_%d" % layer_idx)
            batchnorm = getattr(self, "batchnorm_%d" % layer_idx)
            stride = self.enc_pooling[layer_idx]
            ...
            if self.dropout and layer_idx + 1 != self.enc_layers:
