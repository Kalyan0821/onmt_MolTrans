...
import moltrans_onmt.models.stacked_rnn
import torch
import torch.nn as nn
from moltrans_onmt.utils.misc import aeq
from moltrans_onmt.utils.rnn_factory import rnn_factory
...
class RNNDecoderBase(nn.Module):
    ...
    Args:
       ...
       attn_type (str) : see :obj:`moltrans_onmt.modules.GlobalAttention`
       coverage_attn (str): see :obj:`moltrans_onmt.modules.GlobalAttention`
       context_gate (str): see :obj:`moltrans_onmt.modules.ContextGate`
       copy_attn (bool): setup a separate copy attention mechanism
       dropout (float) : dropout value for :obj:`nn.Dropout`
       embeddings (:obj:`moltrans_onmt.modules.Embeddings`): embedding module to use
    """
    ...
    def __init__(
        ...
        if context_gate is not None:
            self.context_gate = moltrans_onmt.modules.context_gate_factory(
                context_gate, self._input_size, hidden_size, hidden_size, hidden_size
            )

        # Set up the standard attention.
        self._coverage = coverage_attn
        self.attn = moltrans_onmt.modules.GlobalAttention(
            hidden_size,
        ...
        if copy_attn and not reuse_copy_attn:
            self.copy_attn = moltrans_onmt.modules.GlobalAttention(
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None, step=None):
        ...
        Args:
            ...
            state (:obj:`moltrans_onmt.models.DecoderState`):
                 decoder state object to initialize the decoder
            memory_lengths (`LongTensor`): the padded source lengths
                `[batch]`.
        Returns:
            (`FloatTensor`,:obj:`moltrans_onmt.Models.DecoderState`,`FloatTensor`):
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _build_rnn(self, rnn_type, input_size, hidden_size, num_layers, dropout):
        ...
        if rnn_type == "LSTM":
            stacked_cell = moltrans_onmt.models.stacked_rnn.StackedLSTM
        else:
            stacked_cell = moltrans_onmt.models.stacked_rnn.StackedGRU
...
class DecoderState(object):
    ...
    def beam_update(self, idx, positions, beam_size):
        ...
        for e in self._all:
            ...
            sent_states.data.copy_(sent_states.data.index_select(1, positions.long()))
