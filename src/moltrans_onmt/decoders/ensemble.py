...
from moltrans_onmt.models import NMTModel
...
class EnsembleDecoder(nn.Module):
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None, step=None):
        ...
        outputs, states, attns = zip(
            *[
                model_decoder(tgt, memory_bank[i], state[i], memory_lengths, step=step)
                for i, model_decoder in enumerate(self.model_decoders)
            ]
        )
        mean_attns = self.combine_attns(attns)
        return (
            EnsembleDecoderOutput(outputs),
    ...
    def init_decoder_state(self, src, memory_bank, enc_hidden, with_cache=False):
        ...
        return EnsembleDecoderState(
            [
                model_decoder.init_decoder_state(
                    src, memory_bank[i], enc_hidden[i], with_cache
                )
                for i, model_decoder in enumerate(self.model_decoders)
            ]
        )
