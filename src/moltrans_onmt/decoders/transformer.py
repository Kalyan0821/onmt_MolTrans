...
from moltrans_onmt.decoders.decoder import DecoderState
from moltrans_onmt.modules.position_ffn import PositionwiseFeedForward
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout, self_attn_type="scaled-dot"):
        ...
        if self_attn_type == "scaled-dot":
            self.self_attn = moltrans_onmt.modules.MultiHeadedAttention(
                heads, d_model, dropout=dropout
            )
        elif self_attn_type == "average":
            self.self_attn = moltrans_onmt.modules.AverageAttention(d_model, dropout=dropout)

        self.context_attn = moltrans_onmt.modules.MultiHeadedAttention(
            heads, d_model, dropout=dropout
        )
        self.feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.layer_norm_1 = moltrans_onmt.modules.LayerNorm(d_model)
        self.layer_norm_2 = moltrans_onmt.modules.LayerNorm(d_model)
...
class TransformerDecoder(nn.Module):
    ...
    Args:
       ...
       embeddings (:obj:`moltrans_onmt.modules.Embeddings`):
    ...
    def __init__(
        ...
        self._copy = False
        if copy_attn:
            self.copy_attn = moltrans_onmt.modules.GlobalAttention(d_model, attn_type=attn_type)
            self._copy = True
        self.layer_norm = moltrans_onmt.modules.LayerNorm(d_model)

    def forward(
        self, tgt, memory_bank, state, memory_lengths=None, step=None, cache=None
    ):
        """
        See :obj:`moltrans_onmt.modules.RNNDecoderBase.forward()`
        """
        src = state.src
    ...
    def _init_cache(self, memory_bank, num_layers, self_attn_type):
        ...
        for layer_idx in range(num_layers):
            ...
            if self_attn_type == "scaled-dot":
                ...
                layer_cache["self_keys"] = None
                layer_cache["self_values"] = None
            self.cache["layer_{}".format(layer_idx)] = layer_cache
