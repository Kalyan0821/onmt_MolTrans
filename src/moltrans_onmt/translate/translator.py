...
import moltrans_onmt.decoders.ensemble
import moltrans_onmt.inputters as inputters
import moltrans_onmt.model_builder
import moltrans_onmt.opts as opts
import moltrans_onmt.translate.beam
import torch
from moltrans_onmt.utils.misc import tile


def build_translator(
    opt, report_score=True, logger=None, out_file=None, log_probs_out_file=None
):
    if out_file is None:
    ...
    if len(opt.models) > 1:
        ...
        fields, model, model_opt = moltrans_onmt.decoders.ensemble.load_test_model(
            opt, dummy_opt.__dict__
        )
    else:
        fields, model, model_opt = moltrans_onmt.model_builder.load_test_model(
            opt, dummy_opt.__dict__
        )

    scorer = moltrans_onmt.translate.GNMTGlobalScorer(
...
class Translator(object):
    ...
    Args:
       model (:obj:`moltrans_onmt.modules.NMTModel`):
    ...
    def translate(
        ...
        builder = moltrans_onmt.translate.TranslationBuilder(
    ...
    def _fast_translate_batch(
        ...
        alive_seq = torch.full(
            [batch_size * beam_size, 1], start_token, dtype=torch.long, device=device
        )
        alive_attn = None
        ...
        if self.mask is not None:
            mask = self.mask.get_log_probs_masking_tensor(src.squeeze(2), beam_size).to(
                memory_bank.device
            )
        ...
        for step in range(max_length):
            ...
            if return_attention:
                current_attn = attn["std"].index_select(1, select_indices.long())
                if alive_attn is None:
                    alive_attn = current_attn
                else:
                    alive_attn = alive_attn.index_select(1, select_indices.long())
            ...
            if isinstance(memory_bank, tuple):
                ...
                memory_bank = memory_bank.index_select(1, select_indices.long())
            memory_lengths = memory_lengths.index_select(0, select_indices.long())
    ...
    def _translate_batch(self, batch, data):
        ...
        if self.mask is not None:
            mask = self.mask.get_log_probs_masking_tensor(src.squeeze(2), 1).to(
                memory_bank.device
            )
